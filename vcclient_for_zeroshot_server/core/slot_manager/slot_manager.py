...
import shutil
from vcclient_for_zeroshot_server.const import LOGGER_NAME, MAX_SLOT_INDEX, SLOT_PARAM_FILE, ModelDir

from vcclient_for_zeroshot_server.core.data_types.slot_manager_data_types import SeedVCSlotInfo, ModelImportParamMember, MoveModelParam, SetIconParam, SlotInfo, SlotInfoMember
from vcclient_for_zeroshot_server.core.slot_manager.model_importer.model_importer import import_model
...
def load_slot_info(model_dir: Path, slot_index: int) -> SlotInfoMember:
    ...
    try:
        ...
        if tmp_slot_info.vc_type == "seed-vc":
            slot_info: SlotInfoMember = SeedVCSlotInfo.model_validate_json(open(json_file, encoding="utf-8").read())
...
class SlotManager:
    ...
    def set_new_slot(self, model_import_param: ModelImportParamMember, remove_src: bool = False):
        ...
        assert self.get_slot_info(model_import_param.slot_index).vc_type is None, f"slot_index:{model_import_param.slot_index} is already exists."

        logging.getLogger(LOGGER_NAME).info(f"set new slot: {model_import_param}")
        print(f"set new slot: {model_import_param}")
