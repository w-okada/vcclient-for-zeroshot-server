...
from vcclient_for_zeroshot_server.const import LOGGER_NAME, VOICE_CHARACTER_SLOT_PARAM_FILE
from vcclient_for_zeroshot_server.core.data_types.slot_manager_data_types import VoiceCharacter, VoiceCharacterImportParam
...
def import_voice_character(vc_dir: Path, voice_character_import_param: VoiceCharacterImportParam, remove_src: bool = False):
    ...
    try:
        if voice_character_import_param.vc_type == "seed-vc":
            ...
            if voice_character_import_param.zip_file is not None:
                ...
                slot_info.vc_type = voice_character_import_param.vc_type  # ttsのzipはtts_typeしか入っていないため。 TODO: 改善が必要か？
            else:
                # generate config file
                slot_info = VoiceCharacter(
                    vc_type=voice_character_import_param.vc_type,
            ...
            logging.getLogger(LOGGER_NAME).error(f"Unknown vc type: {voice_character_import_param.vc_type}")
            raise RuntimeError(f"Unknown vc type: {voice_character_import_param.vc_type}")
