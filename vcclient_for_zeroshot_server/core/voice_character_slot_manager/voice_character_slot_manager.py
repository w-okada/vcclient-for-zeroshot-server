...
import uuid
import zipfile
from vcclient_for_zeroshot_server.const import LOGGER_NAME, MAX_REFERENCE_VOICE_SLOT_INDEX, MAX_VOICE_CHARACTER_SLOT_INDEX, VOICE_CHARACTER_SLOT_PARAM_FILE, VoiceCharacterDir
from vcclient_for_zeroshot_server.core.data_types.slot_manager_data_types import MoveModelParam, MoveReferenceVoiceParam, ReferenceVoice, ReferenceVoiceImportParam, SetIconParam, VoiceCharacter, VoiceCharacterImportParam
from vcclient_for_zeroshot_server.core.voice_character_slot_manager.importer.importer import import_voice_character
...
def load_slot_info(model_dir: Path, slot_index: int) -> VoiceCharacter:
    ...
    try:
        ...
        if tmp_slot_info.vc_type == "seed-vc":
...
class VoiceCharacterSlotManager:
    ...
    def get_blank_slot_index(self) -> int:
        ...
        exist_slot_index = {slot_info.slot_index for slot_info in self.slot_infos if slot_info.vc_type is not None}
    ...
    def set_new_slot(self, voice_character_import_param: VoiceCharacterImportParam, remove_src: bool = False):
        ...
        assert self.get_slot_info(voice_character_import_param.slot_index).vc_type is None, f"slot_index:{voice_character_import_param.slot_index} is already exists."
    ...
    def update_slot_info(self, slot_info: VoiceCharacter):
        ...
        assert org_slot_info.vc_type is not None, f"src:{slot_info.slot_index} is not exist."
    ...
    def move_model_slot(self, param: MoveModelParam):
        ...
        assert self.get_slot_info(param.dst).vc_type is None, f"dst:{param.dst} is already exists."
        assert self.get_slot_info(param.src).vc_type is not None, f"src:{param.src} is not exist."
    ...
    def get_voice_blank_slot_index(self, voice_character_slot_index: int):
        ...
        assert voice_character.vc_type is not None, f"voice_character_slot_index:{voice_character_slot_index} is not exists."
    ...
    def add_voice_audio(self, index: int, import_params: ReferenceVoiceImportParam, remove_src: bool = False):
        try:
            ...
            assert voice_character.vc_type is not None, f"voice_character_slot_index:{index} is not exists."
    ...
    def delete_voice_audio(self, voice_character_slot_index: int, voice_index: int):
        ...
        try:
            ...
            logging.getLogger(LOGGER_NAME).error(f"Error in deleting voice file: {e}")
            raise e
    ...
    def get_voice_file_path(self, voice_character_slot_index: int, voice_index: int):
        voice_character = self.get_slot_info(voice_character_slot_index)
        assert voice_character.vc_type is not None, f"voice_character_slot_index:{voice_character_slot_index} is not exists."
