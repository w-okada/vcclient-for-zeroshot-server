...
from vcclient_for_zeroshot_server.const import LOGGER_NAME, UPLOAD_DIR, ConfigFile, ModelDir, ModuleDir, VoiceCharacterDir
from vcclient_for_zeroshot_server.server.fileuploader.fileuploader import EasyFileUploader
from vcclient_for_zeroshot_server.server.validation_error_logging_route import ValidationErrorLoggingRoute

from vcclient_for_zeroshot_server.server.server_rest_api_configuration_manager import RestAPIConfigurationManager
from vcclient_for_zeroshot_server.server.server_rest_api_gpu_device_manager import RestAPIGPUDeviceManager
from vcclient_for_zeroshot_server.server.server_rest_api_hello import RestHello

from vcclient_for_zeroshot_server.server.server_rest_api_module_manager import RestAPIModuleManager
from vcclient_for_zeroshot_server.server.server_rest_api_slot_manager import RestAPISlotManager

from vcclient_for_zeroshot_server.server.server_rest_api_vc_manager import RestAPIVCManager
from vcclient_for_zeroshot_server.server.server_rest_api_voice_character_slot_manager import RestAPIVoiceCharacterSlotManager
...
class RestAPI:
    ...
    def get_instance(cls):
        if cls._instance is None:
            ...
            rest_vc_manager = RestAPIVCManager()
            app_fastapi.include_router(rest_vc_manager.router)
            ...
            app_fastapi.add_api_route("/assets/gui_settings/version.txt", get_version, methods=["GET"])

            async def get_edition():
                return PlainTextResponse("python")

            app_fastapi.add_api_route("/assets/gui_settings/edition.txt", get_edition, methods=["GET"])

            async def get_edition_vc():
                return PlainTextResponse("seed-vc")

            app_fastapi.add_api_route("/assets/gui_settings/edition_vc.txt", get_edition_vc, methods=["GET"])
...
def initialize():
    ...
    # VoiceCharacterDirの再生成
    if VoiceCharacterDir.exists():
